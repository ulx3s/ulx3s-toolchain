...
if [ $EXIT_STAT -ne 0 ];then
  echo "Startup ulx3s toolchain. Security Context:" $(whoami) | tee -a  "$THIS_SETUP_LOG"
else
...
if [[ "$1" != *"--nosystem"* ]]  && { [[ "$1" == *"--system"* ]] || [ "$1" == "" ]; } then
	...
	sudo apt-get install build-essential clang bison flex libreadline-dev \
						 ...
						 xdot pkg-config libftdi-dev --assume-yes | tee -a "$THIS_LOG"
	...
	echo "***************************************************************************************************"
	sudo apt-get install python python3 | tee -a "$THIS_LOG"
	sudo apt-get install python3-pip --assume-yes | tee -a "$THIS_LOG"
	pip3 install database

	echo "***************************************************************************************************"
	echo "install apio. Saving log to $THIS_LOG"
	echo "***************************************************************************************************"
	pip3 install -U apio | tee -a "$THIS_LOG"
	CheckForError $? "$THIS_LOG"
...
if [[ "$1" == *"--nextpnr"* ]] || [ "$1" == "" ]; then
	...
	cmake -DARCH=ecp5 -DTRELLIS_INSTALL_PREFIX=/usr .           | tee -a "$THIS_LOG"
	echo This error: $?
...
if [[ "$1" == *"--version"* ]] || [ "$1" == "" ]; then
	...
	python3 --version                                                           | tee -a "$THIS_LOG"
...
if [[ "$1" == *"--ulx3s-bin"* ]] || [ "$1" == "" ]; then
	...
	if [ ! -d "$WORKSPACE"/ulx3s-bin ]; then
	  git clone --recursive https://github.com/ulx3s/ulx3s-bin.git    | tee -a "$THIS_LOG"
	  cd ulx3s-bin
	else
	  cd ulx3s-bin
	  git fetch                                                       | tee -a "$THIS_LOG"
