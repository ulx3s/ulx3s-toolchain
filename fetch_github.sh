...
if [ "$THIS_DIR" != "" ]; then
    ...
    if [ ! -d "$WORKSPACE"/$THIS_DIR ]; then
      ...
      cd $THIS_DIR
      echo ""
      echo "git fetch; # $THIS_REPO_URL"
      git fetch
      if [ "$?" != "0" ]; then
        echo ""
        echo "Error encountered during: git fetch; # $THIS_REPO_URL"
        exit 1
      fi

      echo ""
      echo "git submodule update; # $THIS_REPO_URL"
      git submodule update
      if [ "$?" != "0" ]; then
        echo ""
        echo "Error encountered during: git submodule update; # $THIS_REPO_URL"
        exit 1
      fi

      # before pulling in changes, check out master to ensure we are not detached
      echo ""
      echo "git checkout master; # $THIS_REPO_URL"
      git checkout master
      if [ "$?" != "0" ]; then
        echo ""
      ...
      echo ""
      echo "git pull; # $THIS_REPO_URL"
      echo ""
      git pull
      if [ "$?" != "0" ]; then
        echo ""
        echo "Error encountered during: git pull; # $THIS_REPO_URL"
        echo ""
      ...
      echo ""
      echo "My checkout = $THIS_CHECKOUT"
